  ...
  return (
    <div className="w-full bg-custom-lightGrayColors !-z-30">
      <div className="w-full h-48 xl:h-72 bg-[#F4F4F4] overflow-hidden">
        <img
          className="object-contain object-center cursor-pointer h-full w-full"
          height={"100%"}
          width={"100%"}
      ...
      <div className="md:p-5 p-3">
        <div className="w-full grid grid-cols-6">
          <div className="col-span-4">
            <p
              className="text-base font-normal	text-custom-black line-clamp-2 h-[45px] cursor-pointer"
              onClick={() => {
                sponsoredProduct();
              }}
            >
          ...
          {(item?.is_verified || item?.is_quality) && (
            <div className="col-span-2 justify-end -mr-3  right-0 top-0  flex gap-1 border-0 border-custom-red">
              {item?.is_verified && (
                <MdVerified className="text-3xl text-green-800" />
              )}
              {item?.is_quality && (
                <MdHighQuality className="text-3xl text-custom-red" />
              )}
            </div>
          )}
        </div>
        <div className="pt-2 flex justify-between items-center">
          {item?.price && item?.offer && categoryType === "Products" && (
            <p className="text-lg font-semibold	text-custom-black">
              {constant?.currency}
              {item?.price} <br />{" "}
              <del className="text-custom-lightGrayColor">
                {" "}
                {constant?.currency}
                {item?.offer}
              </del>
          ...
          {item?.attributes[0]?.name &&
            ...
            categoryType === "Business" && (
              <p className="text-lg font-semibold	text-custom-black">
                {item?.attributes[0]?.name} <br />{" "}
                <span className="text-custom-lightGrayColor">
                  {" "}
                  {constant?.currency}
                  {item?.attributes[0]?.value}
                </span>
          ...
          {selectedIndex === -1 && categoryType === "Products" && (
            <button
              type="button"
              className="text-base font-semibold text-custom-black cursor-pointer"
              onClick={() => {
          ...
          {selectedIndex !== -1 && (
            <div className="border border-custom-newOffWhite max-w-max h-[30px] rounded-[15px] md:mt-5 mt-3 flex items-center">
                ...
                onClick={() => {
                  if (cartData[selectedIndex].qty > 1) {
                    const nextState = produce(cartData, (draft) => {
                      ...
                      draft[selectedIndex].total = (
                        draft[selectedIndex]?.price * draft[selectedIndex].qty
                      ).toFixed(2);
                    });
                    setCartData(nextState);
                    localStorage.setItem(
              ...
              {cartData.length > 0 && cartData[selectedIndex]?.qty && (
        ...
        <div className="pt-2 flex justify-start items-center">
          <input
            ...
            onClick={() => {
              ...
              if (!c) {
                if (
                  cartCompareData.length === 0 ||
                  (cartCompareData.length > 0 && checkCat?._id)
                ) {
                  ...
                  toaster({
                    type: "error",
                    message:
                      "You can not add different category in the comparison",
                  });
                }
              } else {
          ...
          />
          <p className="text-base font-semibold text-custom-black  ml-5">
            {t("Add to Compare")}
          </p>
        </div>
      </div>
